...
const t = (id, due) => `('${id}','p','مهمة ${id}','','yellow','progress','خالد','خالد',1,${due ? `'${due}'` : 'NULL'},NULL,NULL,1,1,NULL,NULL)`;
...
test('a single-task reminder attaches that task\'s own action poll', async t2 => {
  ...
  assert.deepEqual(plans[0].choices.options.map(o => o.id), ['TSKsoloFINISH', 'TSKsoloNOTE', 'TSKsoloTRANSFER', 'TSKsoloEXTEND']);
});
