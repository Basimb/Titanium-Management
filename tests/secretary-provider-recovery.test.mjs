...
test('context budget preserves catalogs and complete pending preview',async()=>{
 ...
 // filter, then the message_team-vs-real-task-assignment steering rule (real
 // new capabilities, not bloat) were added to the system prompt -- still a
 // sanity guard against unbounded prompt growth, not a hard model limit.
 assert.deepEqual(sent.tasks,input.tasks);assert.ok(body.messages[0].content.length<23800);
});
