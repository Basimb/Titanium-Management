...
function fixture(t) {
  ...
  db.exec(`PRAGMA foreign_keys=ON;
    ...
    INSERT INTO tasks VALUES('stale','مهمة تجريبية للتأكد من تحديث النظام','تفاصيل','yellow','open','باسم','باسم',1,NULL,NULL,NULL,1,1,NULL,NULL);`);
  migrateSecretary(db);
...
test('a plural bulk close/archive/delete request is refused with a clarify instead of guessing a single target', async t => {
  ...
  assert.match(result.reply, /أكثر من مهمة بنفس الرسالة/);
...
test('other bulk-plural verbs (archive/delete/open/stop) trigger the same guard', async t => {
  ...
  for (const text of ['ارشفهم كلهم', 'احذفهم', 'امسحهم من القائمة', 'افتحهم كلهم', 'وقفهم لحالهم', 'سكرهم بسرعة']) {
    ...
    assert.match(result.reply, /أكثر من مهمة بنفس الرسالة/, text);
  }
});

test('a singular, explicitly named request is unaffected and still reaches the model', async t => {
  const f = fixture(t);
  const chat = async () => emptySecretaryIntent('chat', 'تمام، قفلت مهمة تصليح السيارة.');
  const result = await f.run(f.event('اقفل مهمة تصليح السيارة'), chat);
  assert.equal(result.reply, 'تمام، قفلت مهمة تصليح السيارة.');
});
