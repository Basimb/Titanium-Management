export type DisplayTask = { id:string; title:string; details:string; priority:string; status:string; owner:string|null; suggestedOwner:string|null; dueDate:string|null; archivedAt?:number|null; blocker?:string|null; watcher?:string|null };
...
/** Tasks are standalone records: a live task is simply one that is not archived. */
export function activeTasks(data:DisplayState) { return data.tasks.filter(t=>!t.archivedAt); }
