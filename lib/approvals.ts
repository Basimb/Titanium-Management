...
/** Employee requests responsibility for any active task. Assignment changes only after Basim approves. */
export function requestTaskOwnership(db: DatabaseSync, claimed: ManagementActor, input: { taskId: string; reason?: string }, options: { now?: number } = {}): { approval: Approval; ownerMessage: string } {
  ...
  const approval = insert(db, actor, { type: "task_ownership", entityType: "task", entityId: task.id, summary,
    payload: { taskTitle: task.title, requestedOwnerId: actor.id, requestedOwnerName: actor.name, previousOwner: task.owner, previousSuggestedOwner: task.suggestedOwner, expectedUpdatedAt: task.updatedAt, reason } }, now(options));
