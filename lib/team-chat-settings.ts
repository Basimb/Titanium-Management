...
export const ALLOWED_KEYS = ["TEAM_CHAT_ENABLED", "TEAM_CHAT_SHARED_KEY", "TEAM_CHAT_CONTACTS_JSON", "TEAM_CHAT_GROUP_IDS_JSON", "GROQ_API_KEY", "GROQ_MODEL", "OPENAI_API_KEY", "OPENAI_MODEL", "OPENAI_SEARCH_MODEL",
  ...
  "ODOO_REPORT_ENABLED", "ODOO_URL", "ODOO_DB", "ODOO_USERNAME", "ODOO_API_KEY", "ODOO_LOW_STOCK_THRESHOLD", "ODOO_CURRENCY_LABEL", "ODOO_REPORT_DAILY_HOUR", "ODOO_REPORT_DAILY_MINUTE", "ODOO_REPORT_WEEKLY_DAY", "ODOO_REPORT_WEEKLY_HOUR", "ODOO_REPORT_PURCHASES_WEEKLY_HOUR",
  "ODOO_REPORT_DAILY", "ODOO_REPORT_WEEKLY", "ODOO_REPORT_PURCHASES", "ODOO_QUESTIONS_ENABLED", "ODOO_EXPIRY_WINDOW_DAYS", "OPENAI_ROUTER_MODEL", "ODOO_USER_KEYS_JSON",
  // The clinics. This file is the one place both halves read: the website
  // answers "شو بيع العيادات" from it, and the bridge launcher takes the
  // daily report's settings from it too. Keeping them anywhere else means
  // configuring the same credentials twice and having one half silently not
  // work -- which is exactly what happened on 2026-09-21, when they lived in
  // a file only the bridge could see.
