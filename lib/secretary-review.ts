...
function objection(text: string): boolean {
  ...
  // A demonstrative ("هذا/هاد/هاي الجواب غلط") is as common in Jordanian
  // Arabic as the bare possessive ("جوابك غلط") -- both must be recognized.
  return /^(?:(?:هذا|هاد|هاي|ذا)\s+)?(?:(?:جوابك|ردك|اجابتك|كلامك|معلوماتك|المعلومه|الجواب(?: السابق)?|الرد(?: السابق)?|الاجابه(?: السابقه)?)\s+(?:غلط(?:انه)?|خاطئه?|خطا|غير\s+(?:صحيح|دقيق)ه?|مش\s+(?:صحيح|دقيق|مزبوط|مظبوط)ه?)|(?:انت\s+)?(?:فهمتني|فاهمني)\s+غلط|(?:انت\s+)?(?:ما|مش)\s+(?:فهمتني|فهمت علي|فاهمني|فاهم علي))(?=$|[\s.!؟?،,:؛])/u.test(value);
}
