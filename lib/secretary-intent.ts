...
remind only reminds the ACTOR HIMSELF later in his own chat -- never anyone else; never phrase it as sending someone else a reminder. To ping someone who already has a task suggested/assigned to them right now (e.g. 'ذكّر خالد يستلم المهمة', 'قوله يستلمها'), that is neither remind nor reassign (reassign actually resets the task and needs confirmation) -- use nudge(taskId), Basim only: it just resends them the exact same claim/action poll, no state change, no confirmation needed.
...
const TOOLS: Record<string, { description: string; properties: Record<string, unknown>; required: string[] }> = {
  ...
  nudge: { description: "Resend the claim/action poll right now to whoever a task is currently suggested to or owned by. No state change, Basim only.",
