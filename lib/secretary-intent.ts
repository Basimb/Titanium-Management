...
remind only reminds the ACTOR HIMSELF later in his own chat -- never anyone else; never phrase it as sending someone else a reminder. To nudge someone who already has a task suggested/assigned to them right now (e.g. 'ذكّر خالد يستلم المهمة'), that is not remind -- use command reassign(taskId, same ownerId) if authorized, which resends them the claim poll immediately; otherwise clarify.
