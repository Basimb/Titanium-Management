...
export type SecretaryIntent = {
  kind: "summary" | "details" | "projects" | "report" | "help" | "chat" | "search" | "remind" | "nudge" | "command" | "clarify" | "message_team" | "message_status" | "announce_team" | "task_draft"
...
export type SecretaryModelInput = {
  ...
  taskDraft?: { projectId: string | null; newProjectName: string | null; noProject?: boolean; title: string | null; details: string | null; priority: "red" | "yellow" | "green" | null; ownerId: string | null; dueDate: string | null } | null;
...
const KINDS = ["summary", "details", "projects", "report", "help", "chat", "search", "remind", "nudge", "command", "clarify", "message_team", "message_status", "announce_team", "task_draft",
  "approvals", "decide", "extension", "priority_change", "close_request", "ownership_request", "task_transfer_request", "project_close_request", "rule", "correction", "knowledge", "project_draft"];
...
export function validateSecretaryIntent(value: unknown, input: SecretaryModelInput): SecretaryIntent {
  ...
  if (AGENT_KINDS.has(plan.kind)) {
    ...
    if ((plan.kind === "extension" || plan.kind === "priority_change" || plan.kind === "close_request" || plan.kind === "task_transfer_request") && (plan.taskId === null || !input.tasks.some(t => t.id === plan.taskId))) return emptySecretaryIntent("clarify", "أي مهمة تقصد؟ اذكر اسمها والمشروع.");
  ...
  if (plan.projectId !== null && !(creation && plan.projectId === "no_project") && !input.projects.some(p => p.id === plan.projectId)) return emptySecretaryIntent("clarify", "أي مشروع تقصد؟");
  if (plan.projectId && !(creation && plan.projectId === "no_project")) {
    const normalize = (text: string) => text.normalize("NFKC").replace(/[\u064b-\u065f\u0670\u0640]/g, "").replace(/[أإآ]/g, "ا").replace(/ى/g, "ي").toLowerCase().trim();
...
const TOOLS: Record<string, { description: string; properties: Record<string, unknown>; required: string[] }> = {
  ...
  task_draft: { description: "Start or continue collecting a new task's creation draft. projectId may be the special string no_project for an explicit 'no project' choice (Basim/admin only).",
